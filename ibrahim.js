...
if (conf.ANYWAY_MD === "yes") {
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const dj = '254793859108';
            const dj2 = '254793859108';
            const dj3 = '254793859108';
            const luffy = '254793859108';
         ...
         console.log("lien detecté")
            var verifZokAdmin = verifGroupe ? admins.includes(idBot) : false;
             ...
             if(superUser || verifAdmin || !verifZokAdmin  ) { console.log('je fais rien'); return};
                         ...
                         if ( warn >= warnlimit) { 
                          var kikmsg = `link detected , you will be remove because of reaching warn-limit i do not joke mybe my owner does but me ROLAV1 i do not reach the limit and donnot follow the admin to get you bacak 😠😠😠👌👌🤏`;
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*ROLAV1 WELCOME MESSAGE*`;
            let membres = group.participants;
            for (let membre of membres) {
                msg += ` \n❒ *Hey* 🖐️ @${membre.split("@")[0]} WELCOME TO OUR GROUP. \n\n`;
            }

            msg += `❒ *READ THE GROUP DESCRIPTION TO AVOID GETTING REMOVED* NOT TO FORGET THIS GROUP IS MANAGED BY ROLAV1 DO NOT DO ANYTHING FISHY OK 🤔🤔🤔😁😁😁😃😃😃🤣🤣🤣`;
         ...
         zk.sendMessage(
              group.id,
              {
                text : `@${(group.author).split("@")[0]} has violated the anti-promotion rule, therefore both ${group.author.split("@")[0]} and @${(group.participants[0]).split("@")[0]} have been removed from administrative rights ask the admin to get you back 😞🔙 if he will want to 🤣🤣🤣.`,
           ...
           zk.sendMessage(
                group.id,
                {
                  text : `@${(group.author).split("@")[0]} has violated the anti-demotion rule by removing @${(group.participants[0]).split("@")[0]}. Consequently, he has been stripped of administrative rights🤟🤟🤟🤟 This is ROLAV1 baby am fast 😁😁😁😁.` ,
                  mentions : [group.author,group.participants[0]]
                }
           )
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ ROLAV1 is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ ROLA V1 has Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("Rolav1 is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading Rola  Commands ...\n");
